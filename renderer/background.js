...
export async function loadModel() {
  try {
    ...
    alert(`AI 불러오기 실패:\n${error.message}`);
    let bar = document.getElementById('ai-debug-bar');
    if (!bar) { bar = document.createElement('div'); bar.id = 'ai-debug-bar'; document.body.appendChild(bar); }
...
export function setupBackgroundReplaceModal() {
  ...
  document.querySelectorAll('input[name="background-type"]').forEach(radio => {
    radio.addEventListener('change', e => {
      ...
      if (imageOption) imageOption.style.display = t === 'image' ? 'block' : 'none';
      if (videoOption) videoOption.style.display = t === 'video' ? 'block' : 'none';
      if (colorOption) colorOption.style.display = t === 'color' ? 'block' : 'none';
    });
  });
  ...
  imageFile?.addEventListener('change', e => {
    ...
    reader.onload = ev => {
      const img = new Image();
      img.onload = () => { state.backgroundImage = img; state.backgroundVideo = null; state.bgColor = null; };
  ...
  videoFile?.addEventListener('change', e => {
    ...
    state.backgroundImage = null; state.bgColor = null;
  });

  applyBtn?.addEventListener('click', () => {
    const type = document.querySelector('input[name="background-type"]:checked')?.value;
    if (type === 'none') { state.backgroundImage = null; state.backgroundVideo = null; state.bgColor = null; }
    else if (type === 'color' && colorPicker) { state.bgColor = colorPicker.value; state.backgroundImage = null; state.backgroundVideo = null; }
