...
async function loadUpdateInfo() {
  try {
    const response = await fetch(
      "https://raw.githubusercontent.com/rhm0202/Graduation_Project/develop/updates/latest.json",
    );
    ...
    if (releaseDateFullEl)
      releaseDateFullEl.textContent = formatDate(data.releaseDate);
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const cards = document.querySelectorAll(
    ".feature-card, .download-card, .team-member",
  );
  cards.forEach((card) => {
